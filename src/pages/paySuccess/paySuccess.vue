<template>
  <div>
    <msg title="扣款成功" :description="description" :buttons="buttons" icon="success"></msg>
  </div>
</template>
...
export default {
  ...
  data(){
    ...
    created(){
      ...
      if(this.parmObj[1] === 1){
        this.str = '<p>付款方式：余额</p>'
      }else{
        this.str = '<p>付款方式：积分</p>'
      }
      this.parmObj[1] = decodeURIComponent(this.parmObj[1]) 
      this.description = '<div class="content">'+
                            '<p>付款金额：'+this.parmObj[0]+'</p>'+
                            // <p>付款方式：${this.parmObj[1]}</p>
                            this.str+
                            '<p>订单编号：'+this.parmObj[2]+'</p>'+
                          '</div>'
                          
    AjaxPlugin.$http.get('http://wx.wident.cn/server/mp/signature/wx9962603dfa0540e2?url=http://wx.wident.cn/cfc/paySuccess.html')
    .then((res) => {
      this.init = res.data
      this.$wechat.config({
...
<style lang="scss" src="./paySuccess.scss"></style>
