<template>
  <div>
    <div class="header">
      <p class="header-title">{{accountBalance.amount}}</p>元
    </div>
    <div class="title">最近30条积分记录</div>
    <div class="content">
      <div class="content-item" v-for="item of accountInfo" :key="item.id">
        {{item.remark}}<span class="content-comein" :class="{'content-comeout': item.flag}">{{item.amount}}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created(){
    ...
    AjaxPlugin.$http.get('http://wx.wident.cn/server/mp/signature/wx9962603dfa0540e2?url=http://wx.wident.cn/cfc/accountBalance.html')
    .then((res) => {
      this.init = res.data
      this.$wechat.config({
  ...
  methods: {
    ...
    getBalanceInfo(){
      AjaxPlugin.$http.get(this.baseUrl + '/user/balanceList/' + this.cookies + '/1').then(res => {
...
<style lang="scss" src="./accountBalance.scss"></style>
